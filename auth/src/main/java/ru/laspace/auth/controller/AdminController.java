...
public class AdminController {
        ...
        @Operation(summary = "Разблокировать аккаунт пользователя", description = "Снятие блокировки с аккаунта пользователя (например, после brute force блокировки)")
        @ApiResponses(value = {
        ...
        public ResponseEntity<UserResponse> unlockUserAccount(
                        @Parameter(description = "ID пользователя", required = true) @PathVariable Long id) {
                log.info("Разблокировка аккаунта пользователя ID={}", id);
                UserResponse updatedUser = adminService.unlockUserAccount(id);
                return ResponseEntity.ok(updatedUser);
        }

        @Operation(summary = "Сбросить пароль пользователя", description = "Принудительная смена пароля пользователя")
        @ApiResponses(value = {
        ...
        public ResponseEntity<UserResponse> resetUserPassword(
                        @Parameter(description = "ID пользователя", required = true) @PathVariable Long id,
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "JSON с новым паролем. Пример: {\"newPassword\": \"NewSecurePassword123!\"}", required = true, content = @Content(mediaType = "application/json", examples = @ExampleObject(name = "Пример запроса", value = "{\"newPassword\": \"NewSecurePassword123!\"}", description = "Пример JSON для сброса пароля"), schema = @Schema(implementation = Map.class, example = "{\"newPassword\": \"NewSecurePassword123!\"}"))) @RequestBody Map<String, String> passwordRequest) {
                ...
                log.info("Сброс пароля пользователя ID={}", id);
                UserResponse updatedUser = adminService.resetUserPassword(id, newPassword);
                return ResponseEntity.ok(updatedUser);
        }
}